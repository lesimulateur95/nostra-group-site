export const ROLES_COMMISSIONERS_SETUP_SQL = String.raw`-- NOSTRA GROUP V26 — Rôles multiples + planning commissaires + rapports d'incident
...
set role = case
  when role = 'member' then 'citizen'
  when role in ('staff', 'administrator') then 'employee'
  when role in ('citizen', 'employee', 'commercial', 'commissioner', 'manager') then role
  else 'citizen'
end,
updated_at = now();
...
set roles = array(
  select distinct case
    when value = 'member' then 'citizen'
    when value in ('staff', 'administrator') then 'employee'
    when value in ('citizen', 'employee', 'commercial', 'commissioner', 'manager') then value
    else 'citizen'
  end
  from unnest(roles) as value
), updated_at = now();

update public.member_profiles
set roles = array_append(roles, 'manager'), role = 'manager', updated_at = now()
where discord_id = '331843410962939908' and not ('manager' = any(roles));
...
alter table public.member_profiles
  ...
  check (role in ('citizen', 'employee', 'commercial', 'commissioner', 'manager'));
