...
import {
  getNavigationOrder,
  sortByStoredOrder,
} from "@/lib/content/navigation-order";

export type CustomPageSection = Exclude<
  EditableSiteSection,
  "circuit"
>;
  ...
  motors: [
    {
      ...
      label: "Présentation",
      href: "/motors",
      editableSlug: "motors-presentation",
    },
    {
      key: "catalogue",
      label: "Catalogue",
    ...
    },
    {
      ...
      editableSlug: "motors-fidelite",
    },
    {
      key: "plaques",
      label: "Commander une plaque",
      href: "/motors/plaques",
  ...
  evenements: [
    {
      ...
      label: "Présentation",
      href: "/evenements",
      editableSlug: "evenements-presentation-jeux",
    },
    {
      key: "agenda",
      label: "Agenda",
      href: "/evenements/agenda",
      editableSlug: "evenements-agenda",
    },
    {
      key: "jeux",
      label: "Bingo & Tombola",
      href: "/evenements/jeux",
      editableSlug: "evenements-jeux",
    },
    {
      key: "roue",
      label: "Roue de la chance",
      href: "/evenements/roue-de-la-chance",
      editableSlug: "evenements-roue",
    },
    {
      key: "inscriptions",
      label: "Inscriptions",
...
export function sectionHiddenPagesSlug(
  section: CustomPageSection,
): string {
...
function sectionCategoryPrefix(
  section: CustomPageSection,
): string {
  return `${section}:`;
}

function sectionSlugPrefix(
  section: CustomPageSection,
): string {
  return `${section}-`;
}
...
function toPublicPage(
  ...
  return {
    ...
    category_key: row.category_key.startsWith(
      categoryPrefix,
    )
...
export async function getCustomSectionPages(
  ...
  return ((data ?? []) as RawCustomSectionPage[]).map(
    (row) => toPublicPage(section, row),
  );
}
...
export async function getSectionNavigation(
  ...
  const [customPages, storedOrder, hiddenPageKeys] =
    await Promise.all([
  ...
  for (const page of visible) {
    const current =
      byCategory.get(page.category_key) ?? [];
    current.push(page);
  ...
  for (const category of builtIns) {
    ...
    const builtInVisible = !hiddenPageKeys.has(
      category.editableSlug,
    );

    if (!builtInVisible && attached.length === 0) {
      continue;
    }
  ...
  for (const [categoryKey, pages] of byCategory.entries()) {
    ...
    items.push({
      ...
      href:
        children[0]?.href ??
        `/${section}/personnalise/${first.slug}`,
