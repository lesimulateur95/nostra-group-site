...
export function WheelGame({
  ...
  const paths = useMemo(
    () => segments.map((segment, index) => ({ ...segment, index, path: segmentPath(index, segmentAngle) })),
    [segmentAngle, segments],
  );

  function handleSpin() {
    if (!configured || !enabled || isPending || dailyUsed) return;
    ...
    startTransition(async () => {
      ...
      const targetModulo = (360 - (response.spin.slot_index + 0.5) * segmentAngle + 360) % 360;
      setRotation((current) => {
    ...
    <section className="wheel-game-shell">
      <div className="wheel-stage">
        ...
        <div className="wheel-rotor" style={{ transform: `rotate(${rotation}deg)` }}>
          <svg className="wheel-svg" viewBox="0 0 400 400" role="img" aria-label={`Roue de la chance Nostra Group avec ${segments.length} cases`}>
            <circle cx={CENTER} cy={CENTER} r={196} className="wheel-outer-ring" />
            {paths.map((segment) => {
              const middleAngle = -90 + (segment.index + 0.5) * segmentAngle;
        ...
        <button className="wheel-spin-button" type="button" onClick={handleSpin} disabled={!configured || !enabled || isPending || dailyUsed}>
          {isPending ? "Tirage…" : !enabled ? "ROUE INDISPONIBLE" : dailyUsed ? "TIRAGE DU JOUR UTILISÉ" : "TOURNER LA ROUE"}
        </button>
      </div>

      <aside className="wheel-result-panel" aria-live="polite">
        <p className="eyebrow">RÉSULTAT DU TIRAGE</p>
        {!configured && <div className="wheel-result-empty wheel-result-error"><strong>Mise à jour nécessaire</strong><span>La nouvelle configuration de la roue doit être installée par la Direction.</span></div>}
        {configured && !enabled && !displayedResult && !error && <div className="wheel-result-empty wheel-result-disabled"><strong>Roue temporairement indisponible</strong><span>{disabledMessage}</span></div>}
        {configured && enabled && dailyUsed && !displayedResult && !error && <div className="wheel-result-empty"><strong>Tirage du jour déjà utilisé</strong><span>La roue sera de nouveau disponible à 00h00, heure de France. Les tirages non utilisés ne se cumulent pas.</span></div>}
        {configured && enabled && !dailyUsed && !displayedResult && !error && <div className="wheel-result-empty"><strong>La roue est prête</strong><span>Tu disposes d’un seul tirage par jour, remis à zéro à 00h00.</span></div>}
