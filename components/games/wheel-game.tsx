...
function resultMessage(result: NonNullable<SpinWheelResponse["spin"]>) {
  if (result.prize_type === "loss") return "Cette fois, la roue s’arrête sur Perdu. Tu peux retenter ta chance.";
  return "Ton gain est enregistré dans Mon profil → Jeux. Présente-le à l’équipe lorsqu’il sera utilisé.";
}

export function WheelGame({ configured }: { configured: boolean }) {
  ...
  return (
    <section className="wheel-game-shell">
      <div className="wheel-stage">
        ...
        <button className="wheel-spin-button" type="button" onClick={handleSpin} disabled={!configured || isPending}>
          {isPending ? "Tirage…" : "TOURNER LA ROUE"}
        </button>
      </div>
      ...
      <aside className="wheel-result-panel" aria-live="polite">
        ...
        {configured && !displayedResult && !error && <div className="wheel-result-empty"><strong>La roue est prête</strong><span>Appuie sur le bouton pour lancer un tirage enregistré.</span></div>}
