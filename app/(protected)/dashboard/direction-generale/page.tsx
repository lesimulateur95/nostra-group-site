...
import { getDirectionStatsV155, getRefundsV155, getRentalBookingsV155 } from "@/lib/v155/data";
...
export default async function DirectionGeneralePage(){
  const [kpi,maintenance,stats,refunds,rentals]=await Promise.all([getDirectionOverviewV153(),getMaintenancePolesV153(),getDirectionStatsV155(),getRefundsV155(),getRentalBookingsV155()]);
  return <DashboardShell>
    ...
    <section className={styles.grid}>
      ...
      <article className={styles.kpi}><span>Locations actives</span><strong>{rentals.filter((r:any)=>["confirmed","ready","active"].includes(r.status)).length}</strong></article>
      <article className={styles.kpi}><span>Remboursements à valider</span><strong>{refunds.filter((r:any)=>r.status==="pending").length}</strong></article>
      <article className={styles.kpi}><span>CA location 12 mois</span><strong>{euro(stats.totals.rentals)}</strong></article>
    </section>
    <section className={`${styles.grid2} ${styles.page}`} style={{paddingBottom:0,width:"100%"}}>
      <article className={styles.card}><h2>Centre de pilotage</h2><p>Accès rapide aux modules Direction.</p><div className={styles.actions}>
        <Link className={styles.buttonAlt} href="/dashboard/recherche">Recherche globale</Link><Link className={styles.buttonAlt} href="/dashboard/crm-motors">CRM Motors</Link><Link className={styles.buttonAlt} href="/dashboard/statistiques-avancees">Statistiques avancées</Link><Link className={styles.buttonAlt} href="/dashboard/remboursements">Remboursements</Link><Link className={styles.buttonAlt} href="/dashboard/audit">Audit</Link><Link className={styles.buttonAlt} href="/dashboard/communication">Communication</Link><Link className={styles.buttonAlt} href="/dashboard/codes-promo">Codes promo</Link><Link className={styles.buttonAlt} href="/dashboard/billetterie">Billetterie</Link><Link className={styles.buttonAlt} href="/dashboard/sauvegardes">Sauvegardes</Link>
      </div></article>
