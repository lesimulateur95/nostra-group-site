...
async function staff(){const c=await current();const roles=await getUserRoleKeys(c.user);if(!roles.some(r=>["manager","employee","commercial"].includes(r)))redirect("/accueil");return c;}
...
export async function updateWorkshopCaseV162(form:FormData){
  const {s}=await staff();const id=int(form.get("id"));if(id<=0)redirect("/dashboard/atelier?error=invalid");
...
export async function syncPhysicalStockV162(form:FormData){
  const {s}=await staff();const vehicleId=int(form.get("vehicle_id"));const v=await (s as any).from("catalog_vehicles").select("id,stock_quantity").eq("id",vehicleId).maybeSingle();if(!v.data)redirect("/dashboard/stock-reel?error=vehicle");
...
export async function updatePhysicalUnitV162(form:FormData){const {s}=await staff();const id=int(form.get("id"));const {error}=await (s as any).from("motors_physical_vehicle_units_v162").update({status:text(form.get("status"),40),location:text(form.get("location"),160)||"Stock Nostra Motors",notes:text(form.get("notes"),1000)||null,updated_at:new Date().toISOString()}).eq("id",id);if(error)redirect("/dashboard/stock-reel?error=unit");revalidateV162();redirect("/dashboard/stock-reel?updated=1");}
