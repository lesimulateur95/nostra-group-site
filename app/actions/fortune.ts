...
function text(formData: FormData, name: string, max = 3000) {
...
async function managerRpc(
  ...
  if (error) {
    redirect(`${PATH}?fortune_error=${encodeURIComponent(error.message || "database")}`);
  }
  refresh();
...
export async function createFortuneGame(formData: FormData) {
  const players = [1, 2, 3].map((position) =>
    text(formData, `player_${position}`, 80),
  );
  if (players.some((value) => !value) || new Set(players).size !== 3) {
...
export async function saveFortuneRound(formData: FormData) {
  ...
  if (!gameId || roundNumber < 1 || roundNumber > 4 || category.length < 2 || solution.length < 2) {
...
export async function saveFortuneFinalPuzzle(formData: FormData) {
  const gameId = text(formData, "game_id", 80);
  ...
  await managerRpc(
    "fortune_save_final_puzzle",
    {
      ...
      p_category: category,
      p_solution: solution,
      p_revealed_letters: text(formData, "final_revealed_letters", 30),
    },
    "final-saved",
  );
}

export async function setFortuneWheelVisibility(formData: FormData) {
...
export async function updateFortuneWheelSegment(formData: FormData) {
  ...
  if (
    segmentId <= 0 ||
    !label ||
    !["cash", "bankrupt", "lose_turn", "jackpot", "free_turn", "prize"].includes(kind) ||
...
export async function setFortuneActivePlayer(formData: FormData) {
  const position = integer(formData, "player_position");
  if (position < 1 || position > 3) redirect(`${PATH}?fortune_error=player`);
  await managerRpc(
...
export async function submitFortuneLetter(formData: FormData) {
  ...
  if (!gameId || !["consonant", "vowel"].includes(kind) || !letter) {
  ...
  const { data: result, error } = await (supabase as any).rpc(
    ...
    { p_game_id: gameId, p_letter: letter, p_kind: kind },
  );
  if (error) {
    redirect(`${PATH}?fortune_error=${encodeURIComponent(error.message || "letter")}`);
  }
  const response = result && typeof result === "object" ? (result as Record<string, unknown>) : {};
  refresh();
  redirect(`${PATH}?fortune_success=${Number(response.occurrences) > 0 ? "letter-found" : "letter-missed"}`);
}

export async function submitFortuneSolution(formData: FormData) {
  const gameId = text(formData, "game_id", 80);
  const solution = text(formData, "solution", 300);
  if (!gameId || solution.length < 2) redirect(`${PATH}?fortune_error=solution`);
  const supabase = await createClient();
  const { data } = await supabase.auth.getUser();
  if (!data.user) redirect("/");
  const { data: result, error } = await (supabase as any).rpc(
    "fortune_submit_solution",
    { p_game_id: gameId, p_solution: solution },
  );
  if (error) {
    redirect(`${PATH}?fortune_error=${encodeURIComponent(error.message || "solution")}`);
  }
  const response = result && typeof result === "object" ? (result as Record<string, unknown>) : {};
  refresh();
  redirect(`${PATH}?fortune_success=${response.correct === true ? "solution-correct" : "solution-wrong"}`);
}
