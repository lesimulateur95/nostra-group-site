...
export function EventBoardDisplay({
  ...
  const firstRoundMatchCount = bracket?.rounds[0]?.length ?? 1;
      ...
      <div className={styles.publicBody}>
        {event.format === "bracket" && bracket && (
          <div
            ...
            style={{
              "--rounds": bracket.rounds.length + 1,
              "--bracket-height": `${Math.max(220, firstRoundMatchCount * 116)}px`,
            } as CSSProperties}
          >
            {bracket.rounds.map((matches, roundIndex) => (
              <section className={styles.round} key={roundIndex}>
                ...
                <div
                  ...
                  style={{ "--match-count": matches.length } as CSSProperties}
                >
                  {matches.map((match, matchIndex) => {
                    ...
                    return (
                      <div
                        ...
                        key={matchIndex}
                        style={{
                          top: `${((matchIndex + 0.5) / matches.length) * 100}%`,
