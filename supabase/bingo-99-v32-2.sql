-- NOSTRA GROUP V32.2
-- BINGO AVEC NUMÉROS DE 1 À 99
--
-- Répartition des 5 colonnes :
-- B = 1 à 20
-- I = 21 à 40
-- N = 41 à 60, avec la case centrale Nostra Motors
-- G = 61 à 80
-- O = 81 à 99
--
-- Le carton contient toujours 24 numéros + la case centrale.
...
alter table public.bingo_draws
  ...
  check (ball_number between 1 and 99);
...
-- Les cartons de l'édition active changent de format.
-- On remet donc à zéro uniquement les numéros sortis et les gagnants actuels.
...
delete from public.bingo_draws draw
where draw.round_id in (
  select round.id
  from public.bingo_rounds round
  where round.archived_at is null
);
...
begin
  ...
  if v_ball is null then
    raise exception 'no_numbers_left';
  end if;
