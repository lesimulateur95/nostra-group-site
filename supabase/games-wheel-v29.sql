-- Nostra Group V29 — Roue de la chance, historique et gains citoyens
...
create index if not exists game_wheel_spins_user_id_idx
  on public.game_wheel_spins(user_id, awarded_at desc);
create index if not exists game_wheel_spins_status_idx
  on public.game_wheel_spins(redemption_status, awarded_at desc);
...
create policy "wheel managers delete all"
on public.game_wheel_spins
for delete
to authenticated
using (public.nostra_games_is_manager());
...
grant update, delete on public.game_wheel_spins to authenticated;
